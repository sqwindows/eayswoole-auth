...
```php
     $Auth = new \CrazyCater\EasySwooleAuth\Auth([
          //'auth_on' => true,                // 认证开关
          'session' => $this->session(),    // 认证方式:false为实时认证,传SessionDriver则为session认证。
          'allow_userids' => [10000],  //不用检测权限的用户编号
          'allow_urls' => ['Admin/V1/SystemAuthMenu/tree'], //不用检测权限的URL
     ...
     //默认验证方法
     $check1 = $Auth->check($user_id, $authUrl, $this->request()->getRequestParam());
     var_dump($check1);
    
    //可自定义默认验证地址方式
     $check2 = $Auth->check($user_id, $authUrl, $this->request()->getRequestParam(), function ($info) {
           return $info['module'] . '/' . $info['url'];
         // return $info['module'] . '/'. $info['version'] .'/' . $info['url'];//默认验证地址方式
      });
      var_dump($check2);
